...
if(spLivre.getSelectedItemPosition()>=0){
	...
	db.collection("livres").document(l.getId()).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
		@Override
		public void onSuccess(Void unused) {
			remplir();
			Toast.makeText(getApplicationContext(),"livre supprimer avec succes",Toast.LENGTH_LONG).show();
		}
	}).addOnFailureListener(new OnFailureListener() {
		@Override
		public void onFailure(@NonNull Exception e) {
			Toast.makeText(getApplicationContext(),"Erreur lors de la suppression de livre",Toast.LENGTH_LONG).show();
		}
	});
}
	}
